...
export default function ResetRequestPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // Privacy friendly: do not reveal if email exists
      const { error } = await supabase.auth.resetPasswordForEmail(email.trim(), {
        redirectTo: `${baseUrl}/auth/callback?next=/reset/confirm`,
      });
      if (error) {
        // Show a generic message. Many errors are rate limits or throttling.
  ...
  return (
    <main className="r-background" role="main">
      <section className="r-card" aria-labelledby="r-title">
        ...
        {/* Clean message for expired or invalid links coming back from callback */}
        {invalidLink && (
