...
export default function RecentUploads() {
  return (
    <div className="up-card up-block" aria-labelledby="recent-uploads">
      <h3 id="recent-uploads" className="up-block-title">Recent uploads</h3>
      <div className="up-list" role="list">
        {demo.map((it) => (
          <div key={it.id} className="up-row" role="listitem">
            <div>
              <p className="up-row-title">{it.title}</p>
              <div className="up-row-meta">{it.meta}</div>
            </div>
            <div
              className={`up-status ${it.status === "ready" ? "ready" : it.status === "processing" ? "proc" : "err"}`}
...
function ProcIcon() {
  return (
    <svg viewBox="0 0 24 24" width="14" height="14" aria-hidden="true" className="up-spin">
...
function ErrIcon() {
  return (
    <svg viewBox="0 0 24 24" width="14" height="14" aria-hidden="true">
      <path
        d="M12 9v4m0 4h.01M10.29 3.86l-8.02 13.9A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3.24l-8.02-13.9a2 2 0 0 0-3.42 0z"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
