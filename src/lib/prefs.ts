...
export async function getUserPrefsServer(supabase: SB, userId: string): Promise<UserPrefs> {
  try {
    const { data, error } = await supabase
      .from("user_prefs")
      .select(
        "minutes_goal, default_action, reminder_enabled, reminder_time, reminder_days, timezone, pinned_doc_id, updated_at"
      )
      .eq("user_id", userId)
      .maybeSingle();
    ...
    return {
      ...
      pinned_doc_id: data.pinned_doc_id ?? null,
...
export async function saveUserPrefsClient(supabase: SB, userId: string, prefs: UserPrefs) {
  const payload = {
    ...
    pinned_doc_id: prefs.pinned_doc_id ?? null,
...
export async function setPinnedDocClient(supabase: SB, userId: string, docId: string | null) {
